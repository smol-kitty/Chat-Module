...
const groupChatsModel = {
  ...
  createChat: async (
    ...
    const [result] = await db.query(
      `INSERT INTO group_chats (sender_type, sender_id, receiver_id, message, tagged, files)
     VALUES (?, ?, ?, ?, ?, ?)`,
      [
        sender_type,
        sender_id,
  ...
  updateMessage: async (chat_id, message, tagged = [], files = []) => {
    const [result] = await db.query(
      `UPDATE group_chats
     SET message = ?, tagged = ?, files = ?
     WHERE chat_id = ?`,
      [message, JSON.stringify(tagged), JSON.stringify(files), chat_id]
    );
